# Phase 5 タスク管理 — 施設ナレッジ蓄積（Facility Memory）

> 方針: `report_corrections.diff_json` を店舗・施設単位で集計し、AIが
> **施設固有の事実・既往・レイアウト・運用上の注意点** を施設ナレッジ候補として抽出する（draft）。
> 人間が承認した active ナレッジだけを、事故報告書生成時に
> 「施設固有の参考情報」としてプロンプトに渡す。
>
> `ACCIDENT_REPORT_SYSTEM_PROMPT` 自体は固定のまま変更しない。
> 書きぶり・記法・5W2H の書式などは全社共通ルールとして扱い、施設ナレッジには蓄積しない。
>
> 将来構想: 修正履歴が一定件数貯まったタイミングでの自動生成（週次 cron など）。
> 現状は管理画面からの手動キックのみ。active 化は人間承認後のみ。
