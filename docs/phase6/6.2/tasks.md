...
- [x] `lib/report-search.ts`（新規） — クエリ仕様と SQL 構築
...
- [x] `lib/report-repository.ts` — `searchReports(query: ReportSearchQuery & { facilityId: string })` を追加
  - `getAllReports()` は内部的に `searchReports({ facilityId, status: "all" })` に書き換える形でも可
  - パラメータ化クエリ（SQL injection 回避）
  - LIMIT は当面 200 件程度
- [x] `lib/__tests__/report-search.test.ts` — WHERE 句生成のユニットテスト（pure 関数化したもの）

---

## 3. 検索 API

- [x] `GET /api/reports` を拡張
...
- [x] `app/reports/_components/search-bar.tsx`（新規, Client Component）
...
- [x] `app/reports/_components/report-list-utils.ts` に
...
- [x] サイト A でログイン → 検索バーから「転倒」で検索 → サイト A の該当事故報のみ
- [x] サイト B でログイン → 同じキーワードでもサイト B のレコードしか出ない
- [x] ステータス select で `waiting_human_review` を選ぶと該当のみ
- [x] 日付 from / to で期間絞り込み
- [x] パラメータ無しでアクセスすると現サイトの全件
