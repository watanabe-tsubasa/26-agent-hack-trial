...
function generateCeilingTemplate(
  ...
  return {
    ...
    treatment:
      "該当エリアを一時立入禁止とし、カラーコーン等で区画した。施設管理担当者が現地確認を実施し、落下物の撤去および周辺天井の安全確認を行う。",
    preventiveAction:
      "同一フロアおよび類似箇所の天井ボード固定状態を点検する。天井裏の漏水、設備振動、下地部材の劣化有無を確認する。必要に応じて補修計画を作成する。",
    body: `${whenStr.replace("頃", "")}、${input.location}にて天井ボードの落下が確認された。${hasImages ? `取得画像では、${imageObservation.split("\n")[0].replace(/。$/, "")}。` : ""}現時点で人的被害は${input.hasVictim ? "確認されている" : "確認されていない"}が、二次被害防止のため該当エリアを一時立入禁止とし、施設管理担当者による現地確認を実施する。`,
  };
}
...
function generateGenericTemplate(
  ...
  return {
    ...
    treatment: "該当エリアを確認し、安全確保のための措置を実施した。施設管理担当者による現地確認を実施する。",
    preventiveAction: "類似箇所の安全点検を実施する。必要に応じて補修・改善計画を作成する。",
    body: `${whenStr.replace("頃", "")}、${input.location}にて${input.summary}現時点で人的被害は${input.hasVictim ? "確認されている" : "確認されていない"}。施設管理担当者が現地確認を実施し、安全確保のための初動対応を行う。`,
  };
}
